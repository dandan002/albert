...
def test_base_ingestor_initializes_not_connected():
    bus = EventBus()
    env = {
        "KALSHI_API_KEY_ID": "test_key_id",
        "KALSHI_PRIVATE_KEY": "test_private_key",
    ...
    with patch.dict("os.environ", env):
        with patch("albert.ingestor.kalshi._load_private_key", return_value=make_private_key()):
            ingestor = KalshiIngestor(bus=bus, market_ids=["kalshi:BTC-24"])
    assert ingestor.is_connected is False


async def test_kalshi_ingestor_sets_connected_true_on_connect():
    bus = EventBus()
    ...
    assert ingestor.is_connected is False  # False after finally block exits


async def test_kalshi_ingestor_sets_connected_false_on_exception():
    bus = EventBus()

    mock_ws = AsyncMock()
    mock_ws.__aenter__ = AsyncMock(side_effect=Exception("connection failed"))
...
async def test_polymarket_ingestor_sets_connected_true_on_connect():
    bus = EventBus()
...
async def test_polymarket_ingestor_sets_connected_false_on_exception():
    bus = EventBus()
